...
var api = module.exports = {
    ...
    */
    getFlows: async function(opts) {
        if(opts.req.query.tutorial){
            var tutorialID  = parseInt(opts.req.query.tutorial);
            Log.info("New Flow Request, Tutorial ID: " + tutorialID);
            if(tutorialID > 0 && tutorialID < Tutorial.steps.length){
                var selectedStep = Tutorial.steps[tutorialID]
                return selectedStep.starterFlow;
            } else {
                Log.warn("Invalid Tutorial ID requested: " + tutorialID);
                return { flows: [], rev: "0"}
            }
            
        }

        Log.warn("No TutorialID Provided");
        return { flows: [], rev: "0"}
    },
    /**
    ...
    */
    setFlows: async function(opts) {
        ...
        console.log("Setting FLOWS:", opts)
        return {rev:"1"};
    },
